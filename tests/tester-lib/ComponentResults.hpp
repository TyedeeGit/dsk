    ...
    class ComponentResults {
        private:
            ...
            void update_totals() {
                ...
                for (const auto &[module_name, module_results] : this->module_results) {
                    // Add to the total and passed tests.
                    this->total_tests += module_results.get_total();
                    this->passed_tests += module_results.get_passed();

                    // Increment the total modules.
                    this->total_modules++;

                    // If the module fully passed, increment the passed modules.
                    if (module_results.get_passed() == module_results.get_total()) {
                        this->passed_modules++;
                    }

                    // Add the fails generated by the module.
                    this->fails[module_name] = module_results.get_fails();

                    if (!module_results.get_warnings().empty()) {
                        // Add the warnings generated by the module, if any.
                        this->warnings[module_name] = module_results.get_warnings();
                    }
                }
            }
